...
public class PaymentController {
    ...
    public CommonResult paymentInfo_OK(@PathVariable("id") Integer id) {
        ...
        return CommonResult.ok(paymentService.paymentInfo_OK(id));
    }

    @GetMapping("/hystrix/timeout/{id}")
    public CommonResult paymentInfo_TimeOut(@PathVariable("id") Integer id) {
        log.info("****查询数据的id为:{},端口号为:{}", id, serverPort);
        return CommonResult.ok(paymentService.paymentInfo_TimeOut(id));
    }
    ...
    //*********服务熔断
    @GetMapping("/ciruit/{id}")
    public String paymentCiruitBreaker(@PathVariable("id") Integer id) {
        return paymentService.paymentCiruitBreaker(id);
    }
    /**
     * 新增数据
     *
